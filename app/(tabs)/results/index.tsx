...
const MOCK_RESULTS: EventResult[] = [
  ...
  { id: '2', type: 'entrenamiento', title: 'Sesión Técnica Recepción', date: '2026-04-21' },
    ...
    <SafeAreaView style={tw`flex-1 bg-white`} edges={['top']}>
      ...
      <View style={tw`flex-1 p-5 pt-4`}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={tw`bg-slate-50 p-4 rounded-2xl mb-3 border border-slate-100 shadow-sm`}>
              <Text style={tw`font-bold text-[#003366] text-lg`}>{item.title}</Text>
              <Text style={tw`text-slate-400 text-xs uppercase font-bold`}>{item.date}</Text>
            </View>
          )}
          ListEmptyComponent={
            <Text style={tw`text-center text-slate-400 mt-10`}>No hay resultados registrados</Text>
          }
        />
      </View>
    </SafeAreaView>
