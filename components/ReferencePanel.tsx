...
export default function ReferencePanel({ dark, isOpen, setIsOpen, hoveredAction }: ReferencePanelProps) {
  ...
  return (
    <View style={tw`w-80 h-full ${panelBg} shadow-2xl p-6 border-r ${borderColor}`}>
      ...
      <View style={tw`flex-row justify-between items-center mb-6`}>
        ...
        <TouchableOpacity onPress={() => setIsOpen(false)} style={tw`p-2`}>
          <Ionicons name="chevron-back" size={24} color={dark ? '#94a3b8' : '#003366'} />
        </TouchableOpacity>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {metadata ? (
          ...
          <View>
            <View style={tw`bg-blue-500/10 p-4 rounded-xl mb-6 border border-blue-500/20`}>
              <Text style={tw`text-blue-500 text-[10px] font-black uppercase mb-1`}>Acción Detectada</Text>
              <Text style={tw`text-xl font-black ${textColor}`}>{metadata.name}</Text>
            ...
            <Text style={tw`text-xs font-bold text-slate-500 uppercase mb-4`}>Criterios de Evaluación:</Text>
            ...
            {Object.entries(metadata.criteria)
              ...
              <View key={val} style={tw`mb-4 flex-row items-start`}> 
                  ...
                  <View style={tw`w-8 h-8 rounded bg-slate-800 border border-slate-700 items-center justify-center mr-2 mt-0.5`}>
                  ...
                  <View style={tw`flex-1`}>
                    ...
                    <Text style={tw`text-[13px] ${textColor} font-medium leading-5`}>
          ...
          <View>
            ...
            {[
              ...
              { step: '2', title: 'ACCIÓN', desc: 'Elige la técnica ejecutada (Servicio, Ataque, etc.).' },
              { step: '3', title: 'CALIFICAR', desc: 'Presiona un número (0-4) según el éxito de la acción.' },
              { step: '4', title: 'TRAYECTORIA', desc: 'Marca posición de salida y destino en el campo.' }
            ].map((item) => (
              <View key={item.step} style={tw`mb-6 flex-row items-start`}>
                <Text style={tw`text-2xl font-black text-slate-800 mr-4`}>{item.step}</Text>
                <View style={tw`flex-1`}>
                  <Text style={tw`font-black ${textColor} text-sm mb-1`}>{item.title}</Text>
                  <Text style={tw`text-[11px] ${dark ? 'text-slate-400' : 'text-slate-500'} leading-4`}>{item.desc}</Text>
                </View>
              </View>
            ))}
          </View>
